...
function W.CreateSettings()
    ...
    f:SetSize(510,570); f:SetPoint("CENTER"); f:SetFrameStrata("DIALOG")
    ...
    cycle(p,"fadeAfter","Always-mode fade",{{"OFF","Never"},{"5","After 5 seconds"},{"10","After 10 seconds"}},218)
    note(p,"Fade applies only to Always mode. Hovering the panel pauses it; Pin keeps it open. After fading, move away and back to see the same creature or item again. Player names are excluded.",263)
    p=pages[2]
    ...
    p=pages[3]
    cycle(p,"theme","Style",{{"CLASSIC","Classic"},{"MODERN","Modern"}},0)
    stepper(p,"chineseSize","Chinese text size",1,36)
    stepper(p,"pinyinSize","Pinyin text size",1,72)
    stepper(p,"englishSize","English text size",1,108)
    stepper(p,"width","Panel width",20,144)
    stepper(p,"opacity","Panel opacity",0.05,180,function(v) return math.floor(v*100+0.5).."%" end)
    cycle(p,"anchor","Position",{{"CURSOR","Near initial cursor"},{"FIXED","Fixed position"},{"NAMEPLATE","Above creature"}},218)
    checkbox(p,"locked","Lock reading panel position",256)
    note(p,"Above creature follows its visible nameplate. Hovering the box pauses movement; Pin freezes it. Without a nameplate (or for items), it starts near the cursor. Drag the top edge to set a fixed position.",299)
    p=pages[4]
